...
class HandSignHelper() {
    ...
    var pose: Array<Array<Float>> = Array(25) {Array(2) {0f}} /** 포즈 RAW 좌표 */
    ...
    val frameDeque = ArrayList<FloatArray>().apply {
        repeat(lag) {
            add(FloatArray(174) {0f})
        }
    }
    
    /** 확률을 출력되는 값으로 변경 */
    var signWords : Array<String> = arrayOf(".", "가요", "동대문", "따뜻해요", "마셔요", "먹어요", "마셔요", "오늘", "읽어요", "책", "커피")
...
//        }
    ...
    fun Solution() : ArrayList<FloatArray> {
        ...
        for(i in 0 until 10) {
            result[164 + i] = (resultPose[i] * (180.0 / Math.PI)).toFloat()
        }
