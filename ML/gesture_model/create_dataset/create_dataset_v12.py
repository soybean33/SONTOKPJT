...
def calculate_angle(
    ...
    hand_v1 = joint[start_joint_index, :2]
    hand_v2 = joint[destination_joint_index, :2]
    
    hand_v = hand_v2 - hand_v1
    # Normalize v
    hand_v = hand_v / np.linalg.norm(hand_v, ord=2, axis=1)[:, np.newaxis]
...
def create_data(file_root:str, file_list:list, hand_model_path: str, pose_model_path:str, dataset_type:str, dataset_save_path:str, seq_length:int) -> None:
    ...
    # pose joint setting
    pose_start_joint_index = [0,4,5,6,0,1,2,3,12,14,11,13,11,23]
    pose_destination_joint_index = [4,5,6,8,1,2,3,7,14,16,13,15,12,24]
    pose_angle_index_list_1 = [0,1,2,4,5,6,12,8,12,10]
    ...
    for video in videos:
        ...
        while cap.isOpened():
            ...
            for i in range(len(hand_result.hand_landmarks)):
                f.write("hand"+str(hand_result.hand_landmarks[0][i])+"\n")
            f.write("pose"+str(pose_result.pose_landmarks[0][0])+"\n")
            ...
            if pose_result.pose_landmarks and (hand_result.hand_landmarks or video_label == ''):
                ...
                # 오른손 데이터
                right_hand_data = []
                # 빈값으로 채울 default data
                default = np.zeros(15)
                aa = np.concatenate([np.zeros((21,2)).flatten(), default])
                # 두 손이 체크되었을 때
                ...
                for hand_idx in range(len(hand_result.hand_landmarks)):
                    ...
                    if len(hand_result.hand_landmarks) == 1:
                        ...
                        if hand_result.handedness[hand_idx][0].category_name == 'Left' and hand_result.handedness[hand_idx][0].score >= 0.8:
                            left_hand_data = np.append(left_hand_data,calculate_angle(res, hand_start_joint_index, hand_destination_joint_index, hand_angle_index_list_1, hand_angle_index_list_2))
                            left_check += 1
                        elif hand_result.handedness[hand_idx][0].category_name == 'Right' and hand_result.handedness[hand_idx][0].score >= 0.8:
                            right_hand_data = np.append(right_hand_data,calculate_angle(res, hand_start_joint_index, hand_destination_joint_index, hand_angle_index_list_1, hand_angle_index_list_2))
                            right_check += 1
                    else:
                        if hand_result.handedness[hand_idx][0].category_name == 'Left' and hand_result.handedness[hand_idx][0].score >= 0.8:
                            left_hand_data = np.append(left_hand_data,calculate_angle(res, hand_start_joint_index, hand_destination_joint_index, hand_angle_index_list_1, hand_angle_index_list_2))
                ...
                if len(left_hand_data) == 0:
                    left_hand_data = np.append(left_hand_data,aa)
                if len(right_hand_data) == 0:
                    right_hand_data = np.append(right_hand_data,aa)
                frame_data = np.append(frame_data, left_hand_data)
                frame_data = np.append(frame_data, right_hand_data)
                # pose
                # pose anlge 계산 후 저장
        ...
        if video_data.size >= 10:
            ...
            if len(video_data) > seq_length:
                ...
                video_data = np.concatenate((video_data, video_data), axis=0)
                for seq in range(len(video_data) - seq_length):
                    full_seq_data.append(video_data[seq:seq + seq_length])
...
def dir_setting() -> list:
    ...
    seq_length_input = input("legs (default 5) : ").strip()
    # input이 있을 경우만 설정
    if type_input != '':
    ...
    return [file_root, file_list, hand_model_path, pose_model_path, dataset_type, dataset_save_path, int(seq_length)]
