...
def create_data(file_root:str, file_list:list, hand_model_path: str, pose_model_path:str) -> None:
    ...
    # pose
        ...
        if video_data.size >= 10:
            ...
            print("saved!")
            np.save(os.path.join('dataset', f'seq_{video[0:-4]}_{video_label}'), video_data)
        ...
        # action_idx += 1
    # 라벨 데이터 저장
    np.save(os.path.join('dataset', f'labels'), list(actions.keys().sort(key=lambda x:actions[x])))
